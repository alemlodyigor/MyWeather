import View from "./view.js";

class weatherView extends View {
    _parentElement = document.querySelector('.actual-container');
    _errorMessage = 'Could not find this country. Try again!';
    _message;

    _generateWeather(){
        return `
            <div class="actual-container__actual__state">
            <h3>${this._date.location.name}, &nbsp;</h3>
            <span>${this._date.language.lastUpdated} ${this._displayHour(this._date.current.last_updated_epoch)}</span>
            </div>
            <h2 class="actual-container__actual__temp">${this._date.current.temp_c}℃</h2>

            <img src="${this._date.current.condition.icon}" alt="" class="actual-container__actual__image">

            <h3 class="actual-container__actual__condition">${this._date.current.condition.text}</h3>

            <div class="actual-container__actual__astro">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="icon icon--sun">
                <path stroke-linecap="round" stroke-linejoin="round" d="M12 3v2.25m6.364.386l-1.591 1.591M21 12h-2.25m-.386 6.364l-1.591-1.591M12 18.75V21m-4.773-4.227l-1.591 1.591M5.25 12H3m4.227-4.773L5.636 5.636M15.75 12a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0z" /></svg>
                <p>${this._date.forecast.forecastday[0].astro.sunrise} - ${this._date.forecast.forecastday[0].astro.sunset}</p>

                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="icon">
                <path stroke-linecap="round" stroke-linejoin="round" d="M18 12H6" /></svg>

                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="icon icon--moon">
                <path stroke-linecap="round" stroke-linejoin="round" d="M21.752 15.002A9.718 9.718 0 0118 15.75c-5.385 0-9.75-4.365-9.75-9.75 0-1.33.266-2.597.748-3.752A9.753 9.753 0 003 11.25C3 16.635 7.365 21 12.75 21a9.753 9.753 0 009.002-5.998z" /></svg>                    
                <p>${this._date.forecast.forecastday[0].astro.moonset} - ${this._date.forecast.forecastday[0].astro.moonrise}</p>
            </div>
        `;
    }
}

export default new weatherView();